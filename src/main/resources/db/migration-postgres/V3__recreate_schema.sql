              -- V3__recreate_schema.sql
              -- DANGER: This migration DROPS and RECREATES the entire "public" schema.
              -- Use for a one-time reset only, then remove/rename this migration.

              -- 1) Ensure schema exists & set search_path
              CREATE SCHEMA IF NOT EXISTS public;
              SET search_path TO public;

              -- 2) Drop all dependent objects cleanly
              DO $drop$
              DECLARE
                r RECORD;
              BEGIN
                -- drop views (regular & materialized), then tables, then sequences not owned by tables
                FOR r IN (SELECT schemaname, matviewname AS viewname, 'MATERIALIZED VIEW' AS kind
                            FROM pg_matviews WHERE schemaname = 'public'
                          UNION ALL
                          SELECT table_schema AS schemaname, table_name AS viewname, 'VIEW' AS kind
                            FROM information_schema.views WHERE table_schema = 'public')
                LOOP
                  EXECUTE format('DROP %s IF EXISTS %I.%I CASCADE', r.kind, r.schemaname, r.viewname);
                END LOOP;

                -- drop functions (to avoid trigger-function conflicts)
                FOR r IN
                ...
                -- drop leftover sequences (those not dropped with their tables)
                FOR r IN
                ...
                LOOP
                  EXECUTE format('DROP SEQUENCE IF EXISTS %I CASCADE', r.seqname);
                END LOOP;
              END
              $drop$;

              -- 3) Recreate schema matching entities
